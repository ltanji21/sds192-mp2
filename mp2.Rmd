...
```{r}
job_names2 <- c("YOGA", "SOCIAL WORKER", "DOCTOR", "PHYSICIAN", "LAWYER", "ATTORNEY")

occupations_people2 <- function(job_arg) {
  individuals %>%
  group_by(state) %>%
  summarise(people_no = n()) %>%
  filter(people_no >= 50, state != "ZZ") %>%
...
lapply(job_names2, FUN = occupations_people2)
```


Making 'occupations_donation' into a function. 
```{r}
occupations_donation2 <- function(job_arg) {
  individuals %>%
  group_by(state) %>%
  summarise(total_donation2 = sum(transaction_amt)) %>%
  filter(total_donation2 > 1000, !is.na(state)) %>%
  select(contains(job_arg), state, total_donation2)
}

lapply(job_names2, FUN = occupations_donation2)

occupations_donation2
```


Now making 'donations_occupation' into a function. 
```{r}
donations_occupation2 <- function() {
  occupations_people %>%
  left_join(occupations_donation2, by = "state") %>%
  mutate(average_donation2 = total_donation/people_no) %>%
  select(state, average_donation2)
}
