...
Dem_states <- percentage_parties %>%
  group_by(Dem) %>%
  summarise(state_no = n()) %>%
  select(state_no, Dem) %>%
  ggplot(aes(x = Dem, y = state_no)) +
  geom_jitter(stat = "identity") + xlim(10,101) + ylim(0,4.1)
...
Rep_states <- percentage_parties %>%
  ...
  select(state_no2, Rep) %>%
  ggplot(aes(x = Rep, y = state_no2)) +
  geom_jitter(stat = "identity") + xlim(0,30) + ylim(0,3.1)
